...
class JumpGameUp
{
	...
	private $_height = 0;
	private $_CONF = [];
	private $_alphaName = './image/alpha1.png';
	private $_alphaName2 = './image/alpha2.png';
	private $_coordinate = [];
...
//				$this->drawCircle($this->_image, "./image/{$id}_img.png", $this->_coordinate['x'], $this->_coordinate['y'], 10, 10, 0, 0, 0, 100);
//				$this->drawCircle($this->_image, "./image/{$id}_img.png", $this->_chessboardCoordinate['x'], $this->_chessboardCoordinate['y'], 10, 10, 0, 0, 0, 100);
		...
		for ($y = $this->_height / 3; $y < $this->_height / 4 * 3; $y++) {
			...
			for ($x = 0; $x < $this->_width; $x++) {
				...
				if (abs($RGB['red'] - $this->_CONF['CHESS_RGB']['r']) < $this->_CONF['CHESS_RGB']['diff']
					...
					$coordinates[] = ['x' => $x, 'y' => $y];
//					$this->drawCircle($this->_image, './image/screen4.png', $x, $y, 1, 1, 255, 0, 0, 100);
				}
			}
		}
	...
	public function alphaImage(array $coordinateTop): array
	{
		...
		$this->drawCircle($this->_image, $this->_alphaName, $coordinateTop['x'], $coordinateTop['y'], 60, 200, 0, 0, 0, 0);

//		$alpha = (imagecolorat($this->_image, $x, $y) >> 24 & 0xFF);
...
//
		...
		$this->drawCircle($this->_image, $this->_alphaName2, $coordinateTop['x'], $coordinateTop['y'], 1, 1, 0, 0, 0, 0);
		//棋子极点去除
